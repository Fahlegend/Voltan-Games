...
function cardFor(item, category, index) {
  ...
  const source = safeURL(item.image || (typeof item.images?.[0] === 'string' ? item.images[0] : item.images?.[0]?.src));
  ...
  const url=category==='games'?safeURL(item.link):'Product.html?category='+encodeURIComponent(category)+'&id='+encodeURIComponent(item.id);
  if(url){const link=document.createElement('a');link.className='btn';link.href=url;link.textContent=category==='games'?'View game details ↗':'View product details ↗';if(new URL(url,location.href).origin!==location.origin){link.target='_blank';link.rel='noopener noreferrer';}content.append(link);}
...
readJSON('ProductData.json').then(data=>{for(const category of ['printing','merch']){if(!Array.isArray(data[category])){catalogs[category].error=true;continue;}catalogs[category].items=data[category].filter(item=>item&&typeof item.name==='string').map((item,index)=>({...item,id:item.id||category+'-'+index}));catalogs[category].ready=true;}}).catch(()=>{catalogs.printing.error=true;catalogs.merch.error=true;}).finally(renderAll);
renderAll();
